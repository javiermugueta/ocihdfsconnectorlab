...
CREATE EXTERNAL TABLE IF NOT EXISTS banking.transactions_csv_app (
  txn_id STRING,
  txn_date STRING,
  account_id STRING,
  customer_id STRING,
...
DROP TABLE IF EXISTS banking.account_risk_hdfs;

CREATE TABLE banking.account_risk_hdfs AS
SELECT
  account_id,
  txn_date,
  COUNT(*) AS txn_count,
  ...
  CASE
    ...
    ELSE 'LOW'
  END AS risk_level
FROM banking.transactions_csv_app
